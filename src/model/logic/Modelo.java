...
public class Modelo {
    ...
    public String req1String(String punto1, String punto2) {
        ...
        try {
            ...
            Vertex vertice1 = getFirstVertexFromLandingId(codigo1);
            Vertex vertice2 = getFirstVertexFromLandingId(codigo2);
            int elemento1 = (int) tabla.get(vertice1.getId());
            int elemento2 = (int) tabla.get(vertice2.getId());
            fragmento += areInSameCluster(elemento1, elemento2);
        } catch (PosException | VacioException e) {
            e.printStackTrace();
        }
        return fragmento;
    ...
    private int findMaxInList(ILista lista) {
        int max = 0;
        for (int i = 1; i <= lista.size(); i++) {
            try {
                int element = (int) lista.getElement(i);
                if (element > max) {
                    max = element;
                }
            } catch (PosException | VacioException e) {
    ...
    private Vertex getFirstVertexFromLandingId(String codigo) throws PosException, VacioException {
        return (Vertex) ((ILista) landingidtabla.get(codigo)).getElement(1);
    }

    private String areInSameCluster(int elemento1, int elemento2) {
    ...
    public String req2String() {
        ...
        String fragmento = "";
        int contador = 0;
        for (int i = 1; i <= lista.size() && contador <= 10; i++) {
            try {
                ILista element = (ILista) lista.getElement(i);
                if (element.size() > 1) {
                    fragmento += processElement(element);
                    contador++;
                }
            } catch (PosException | VacioException e) {
                e.printStackTrace();
    ...
    private String processElement(ILista element) throws PosException, VacioException {
        Landing landing = (Landing) ((Vertex) element.getElement(1)).getInfo();
        int cantidad = calculateCantidad(element);
        return "\n Landing " + "\n Nombre: " + landing.getName() + "\n País: " + landing.getPais() + "\n Id: " + landing.getId() + "\n Cantidad: " + cantidad;
    }

    private int calculateCantidad(ILista element) throws PosException, VacioException {
        int cantidad = 0;
        for (int j = 1; j <= element.size(); j++) {
            cantidad += ((Vertex) element.getElement(j)).edges().size();
        }
        return cantidad;
    }
    ...
    public String req4String() {
        try {
            ...
            String llave = findMaxLlave(lista1);
            ILista lista2 = grafo.mstPrimLazy(llave);
            Result result = processLista2(lista2);
            return generateResultString(result);
    ...
    private String findMaxLlave(ILista lista1) throws PosException, VacioException {
        int max = 0;
        String llave = "";
        for (int i = 1; i <= lista1.size(); i++) {
            ILista element = (ILista) lista1.getElement(i);
            if (element.size() > max) {
                max = element.size();
                llave = (String) ((Vertex) element.getElement(1)).getId();
            }
        }
        return llave;
    }

    private Result processLista2(ILista lista2) throws PosException, VacioException, NullException {
        ...
        PilaEncadenada caminomax = findMaxPath(unificado, tabla);
        return new Result(unificado.size(), distancia, caminomax);
    }

    private PilaEncadenada findMaxPath(ILista unificado, ITablaSimbolos tabla) throws PosException, NullException, VacioException {
        int maximo = 0;
    ...
    private String generateResultString(Result result) {
        String fragmento = " La cantidad de nodos conectada a la red de expansión mínima es: " + result.size + "\n El costo total es de: " + result.distancia;
        fragmento += "\n La rama más larga está dada por lo vértices: ";
        for (int i = 1; i <= result.caminomax.size(); i++) {
            Vertex pop = (Vertex) result.caminomax.pop();
            fragmento += "\n Id " + i + " : " + pop.getId();
        }
        return fragmento;
    }
    ...
    private static class Result {
        int size;
        int distancia;
        PilaEncadenada caminomax;

        Result(int size, int distancia, PilaEncadenada caminomax) {
            this.size = size;
            this.distancia = distancia;
            this.caminomax = caminomax;
        }
    }
    ...
    public ILista req5(String punto) {
        ...
        addOriginalCountry(countries, lista);

        processVertices(countries, lista);

        ILista unificado = unificar(countries, "Country");

        sortCountries(unificado, lista);

        return unificado;
    }

    private void addOriginalCountry(ILista countries, ILista lista) {
        try {
    ...
    private void processVertices(ILista countries, ILista lista) {
        for (int i = 1; i <= lista.size(); i++) {
            try {
                ...
                for (int j = 1; j <= arcos.size(); j++) {
                    ...
                    processVertex(countries, vertice2);
                }
    ...
    private void processVertex(ILista countries, Vertex vertice2) throws PosException, VacioException, NullException {
        Country pais = null;
    ...
    private void sortCountries(ILista unificado, ILista lista) {
    ...
    public <T extends Comparable<T>> ILista<T> unificar(ILista<T> lista, String criterio) {
        ...
        Comparator<T> comparador = null;

        if (criterio.equals("Vertice")) {
            comparador = Comparable::compareTo;
        } else {
            comparador = (Comparator<T>) new Country.ComparadorXNombre();
        }
        ...
        try {
            ...
            for (int i = 1; i <= lista.size(); i++) {
                T actual = lista.getElement(i);
                T siguiente = (i < lista.size()) ? lista.getElement(i + 1) : null;
                T anterior = (i > 1) ? lista.getElement(i - 1) : null;

                if ((siguiente != null && comparador.compare(actual, siguiente) != 0) ||
                        (siguiente == null && anterior != null && comparador.compare(anterior, actual) != 0) ||
    ...
    private void cargarPuntosDeAterrizaje() throws IOException {
    ...
    private void manejarExcepcion(Exception e) {
        e.printStackTrace(); // Cambia esto según el manejo que desees hacer con las excepciones
    }
